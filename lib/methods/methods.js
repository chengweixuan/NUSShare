if(Meteor.isServer){
  Meteor.methods({
    ...
    updateUsername: function(username) {
      Threads.update( {userId: Meteor.user()._id}, {$set:{"author": username}}, {multi: true});
      Posts.update( {userId: Meteor.user()._id}, {$set:{"author": username}}, {multi: true});
      Comments.update( {userId: Meteor.user()._id}, {$set:{"author": username}}, {multi: true});
      Questions.update( {userId: Meteor.user()._id}, {$set:{"author": username}}, {multi: true});
      ProfileComments.update( {userId: Meteor.user()._id}, {$set:{"author": username}}, {multi: true});
    },

    anonOn: function(anon) {
      var newname = Meteor.user().username + " (" + Meteor.user().profile.realname + ")";
      Threads.update( {userId: Meteor.user()._id}, {$set:{"author": newname}}, {multi: true});
      Posts.update( {userId: Meteor.user()._id}, {$set:{"author": newname}}, {multi: true});
      Comments.update( {userId: Meteor.user()._id}, {$set:{"author": newname}}, {multi: true});
      Questions.update( {userId: Meteor.user()._id}, {$set:{"author": newname}}, {multi: true});
      ProfileComments.update( {userId: Meteor.user()._id}, {$set:{"author": newname}}, {multi: true});
    },

    anonOff: function(anon) {
      var original = Meteor.user().username.length;
      var newname = Meteor.user().username.slice(0, original);
